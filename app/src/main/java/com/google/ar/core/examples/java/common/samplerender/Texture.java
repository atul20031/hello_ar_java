...
import static com.google.ar.core.examples.java.helloar.HelloArActivity.image_bitmap;
import static com.google.ar.core.examples.java.helloar.HelloArActivity.image_loaded;
...
public class Texture implements Closeable {
   ...
   * mostly only useful for creating {@link} textures. See {@link
  ...
  public static Texture createFromAsset(SampleRender render, String assetFileName, WrapMode wrapMode, ColorFormat colorFormat, int url)
    throws IOException
  {
    Texture texture = new Texture(render, Target.TEXTURE_2D, wrapMode);
    Bitmap bitmap = null;
    try{
      Log.d("Path","Inside Texture.java new function  "+assetFileName);
        Bitmap temp=null;
        try {
          if(url == 5)
          {

            temp = getBitmapFromURL(assetFileName);
//            BitmapFactory.Options options = new BitmapFactory.Options();
//            options.inPreferredConfig = Bitmap.Config.ARGB_8888;
//            temp = image_bitmap;
            Log.d("Path","temp set to image_bitmap");
          }
          else {
        ...
        {
          e.printStackTrace();
          Log.d("Path","Failed decoding stream : "+e.toString());
        }
          bitmap =
          ...
          Log.d("Path","successfully created texture for "+assetFileName);
    } catch (Exception e)
    {
      Log.d("Path",e.toString());
      Log.d("Path","Problem in creating texture for "+assetFileName);
      texture = createFromAsset(render,"models/AA.jpg",wrapMode,colorFormat);

    } catch (Throwable t)
    {
      Log.d("Path","problem in creating texture");

      throw t;
    } finally {
      if (bitmap != null) {
        Log.d("Path","Inside Finally");
        bitmap.recycle();
      }
    }
    return texture;
  ...
  public static Texture createFromAsset(
    ...
    Texture texture = new Texture(render, Target.TEXTURE_2D, wrapMode);
    Bitmap bitmap = null;
    Log.d("Path","Inside original function "+assetFileName);
    ...
    try {
      ...
      Log.d("Path",render.getAssets().toString());
      bitmap =
          convertBitmapToConfig(
